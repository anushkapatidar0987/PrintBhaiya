...
export default function NewOrder({ currentUser }) {
  ...
  // UI Local Price Estimate
  const calculatePrice = (shop) => {
    if (!shop || !shop.price_list) return 0;
    const pl = shop.price_list;
    let pageRate = printType === 'COLOR' || printType === 'color' ? pl.color_rate_per_page : pl.bw_rate_per_page;
    ...
    if (bindingOption && bindingOption !== 'None') {
      const option = shop.binding_options.find(b => b.id === bindingOption || b.name === bindingOption);
      if (option) bindingCost = parseFloat(option.price);
    }
  ...
  const handlePayClick = async () => {
    ...
    try {
      ...
      setPaymentStatus('pending'); // Ready for mock checkout
  ...
  const simulatePaymentSuccess = async () => {
    ...
    try {
      ...
      // 2. Mock Verify (bypassing signature check due to RAZORPAY_MOCK_MODE=True in backend)
                ...
                return (
                  ...
                  >
                    ...
                    <div style={styles.shopRatesLine}>
                      <span>B&W: ₹{shop.price_list?.bw_rate_per_page}/pg</span>
                      <span>Color: ₹{shop.price_list?.color_rate_per_page}/pg</span>
                    </div>

                    {isOpen && (
                      <div style={styles.shopPricePreview}>
                        <span>Calculated Price:</span>
                        <span style={styles.previewPriceValue}>₹{Number(calculatedCost || 0).toFixed(2)}</span>
                      </div>
                    )}
                  </div>
                );
          ...
          <div className="card">
            ...
            <div style={styles.priceSummaryBox}>
              ...
              <div style={styles.breakdownRow}>
                <span>Prints ({pageCount * copies} pgs × ₹{Number(printType === 'color' ? selectedShop.price_list?.color_rate_per_page : selectedShop.price_list?.bw_rate_per_page || 0).toFixed(2)})</span>
                <span>₹{Number(pageCount * copies * (printType === 'color' ? selectedShop.price_list?.color_rate_per_page : selectedShop.price_list?.bw_rate_per_page)).toFixed(2)}</span>
              </div>
              {bindingOption !== 'None' && (
                <div style={styles.breakdownRow}>
                  <span>Binding Charge ({bindingOption})</span>
                  <span>₹{Number(selectedShop.binding_options?.find(b => b.name === bindingOption)?.price || 0).toFixed(2)}</span>
                </div>
              )}
              <div style={styles.breakdownRow}>
            ...
            <div style={styles.wizardNav}>
              ...
              <button onClick={handlePayClick} className="btn btn-accent" style={{fontSize: '1.2rem', padding: '0.85rem 2rem'}}>
                💳 Pay ₹{Number(totalPrice || 0).toFixed(2)} via Razorpay
              </button>
            </div>
          </div>
        )}
          ...
          <div style={styles.modalContent}>
            ...
            <div style={styles.razorpayHeader}>
              ...
              <div style={styles.razorpayAmountCol}>
                ...
                <span style={{fontSize: '1.4rem', fontWeight: '700', color: '#FFF'}}>₹{Number(totalPrice || 0).toFixed(2)}</span>
              </div>
            </div>
