...
export default function StudentDashboard({ currentUser }) {
  ...
  const loadOrders = async () => {
    try {
      const response = await orderService.getStudentOrders();
      ...
      if (selectedOrder) {
        ...
        const active = list.find(o => o.status !== 'READY_FOR_COLLECTION' && o.status !== 'REJECTED');
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      ...
      case 'ACCEPTED':
        return <span className="badge badge-info">Accepted &amp; Processing</span>;
  ...
  const getStepStatus = (orderStatus, step) => {
    // Steps: 1: PLACED, 2: ACCEPTED, 3: READY_FOR_COLLECTION
    const statusMap = {
      'PENDING_PAYMENT': 0,
      'PLACED': 1,
      'ACCEPTED': 2,
                ...
                >
                  <div style={styles.orderCardHeader}>
                    <div>
                      ...
                      <h4 style={styles.shopName}>{order.shop_name}</h4>
                    </div>
                    {getStatusBadge(order.status)}
                  </div>
                  
                  <div style={styles.orderCardDetails}>
                    <span style={styles.fileDetail}>📄 {order.files?.[0]?.original_filename} ({order.page_count} pgs × {order.copies} copies)</span>
                    <span style={styles.orderPrice}>Total: <strong>₹{Number(order.total_amount || 0).toFixed(2)}</strong></span>
                  </div>
                </div>
              ))
            )}
                ...
                {selectedOrder.status === 'REJECTED' ? (
                  ...
                  <div style={styles.stepperContainer}>
                    ...
                    <div style={styles.stepItem}>
                      ...
                      <div style={styles.stepText}>
                        <h4 style={styles.stepTitle}>Accepted &amp; Processing</h4>
                        <p style={styles.stepDesc}>Shopkeeper has accepted your order and is printing it.</p>
                      </div>
                    </div>

                    {/* Step 3: Ready for Collection */}
                    <div style={{...styles.stepItem, opacity: getStepStatus(selectedOrder.status, 3) === 'pending' ? 0.4 : 1}}>
                      <div style={styles.stepConnectorLine}></div>
                      <div style={{
                        ...styles.stepIndicator,
                      ...
                      <div style={styles.stepText}>
                        ...
                        <p style={styles.stepDesc}>Head to the shop! Your prints are ready to be picked up.</p>
                      </div>
                    </div>
                  </div>
                )}
                ...
                <table style={styles.specsTable}>
                  <tbody>
                    <tr>
                      ...
                      <td style={styles.tdValue}>{selectedOrder.shop_name}</td>
                    </tr>
                    <tr>
                      <td style={styles.tdLabel}>File:</td>
                      <td style={styles.tdValue} title={selectedOrder.files?.[0]?.original_filename}>{selectedOrder.files?.[0]?.original_filename}</td>
                    </tr>
                    <tr>
                      <td style={styles.tdLabel}>Print Type:</td>
                      <td style={styles.tdValue} style={{textTransform: 'uppercase'}}>{selectedOrder.color_mode}</td>
                    </tr>
                    <tr>
                      <td style={styles.tdLabel}>Pages / Copies:</td>
                      <td style={styles.tdValue}>{selectedOrder.page_count} pgs × {selectedOrder.copies} copies</td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                      ...
                      <td style={styles.tdValue} style={{fontWeight: 'bold'}}>₹{Number(selectedOrder.total_amount || 0).toFixed(2)}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
