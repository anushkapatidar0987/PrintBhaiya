...
export default function ShopDashboard({ currentUser }) {
  ...
  const loadShopData = () => {
    const ownerShop = mockDb.getShopByOwnerId(currentUser.id);
    if (ownerShop) {
      setShop(ownerShop);
      setPriceBw(ownerShop.pricePerBw);
      setPriceColor(ownerShop.pricePerColor);
      
      const spiral = ownerShop.bindingOptions.find(b => b.name === 'Spiral Binding');
      const staple = ownerShop.bindingOptions.find(b => b.name === 'Stapled');
      setSpiralPrice(spiral ? spiral.price : 30);
      setStaplePrice(staple ? staple.price : 5);

      const list = mockDb.getOrdersByShop(ownerShop.id);
      setOrders(list);
    }
  };
  ...
  const handleStatusToggle = (newStatus) => {
    if (shop) {
      const updated = mockDb.updateShopStatus(shop.id, newStatus);
      setShop(updated);
    }
  };

  const handleAcceptOrder = (orderId) => {
    mockDb.updateOrderStatus(orderId, 'ACCEPTED');
    loadShopData();
  };
  ...
  const handleConfirmReject = () => {
    if (rejectOrderId && rejectReason) {
      mockDb.updateOrderStatus(rejectOrderId, 'REJECTED', rejectReason);
  ...
  const handleMarkPrinting = (orderId) => {
    mockDb.updateOrderStatus(orderId, 'PRINTING');
    loadShopData();
  };

  const handleMarkReady = (orderId) => {
    mockDb.updateOrderStatus(orderId, 'READY');
    loadShopData();
  };

  const handleMarkCollected = (orderId) => {
    mockDb.updateOrderStatus(orderId, 'COLLECTED');
    loadShopData();
  };
  ...
  const handlePricingSave = (e) => {
    ...
    if (shop) {
      const bindingOptions = [
        { id: "b1", name: "Spiral Binding", price: parseFloat(spiralPrice) || 0, active: true },
        { id: "b2", name: "Stapled", price: parseFloat(staplePrice) || 0, active: true }
      ];
      mockDb.updateShopPricing(shop.id, priceBw, priceColor, bindingOptions);
  ...
  const filteredOrders = () => {
    switch (queueFilter) {
      ...
      case 'active':
        return orders.filter(o => o.status === 'ACCEPTED' || o.status === 'PRINTING');
      case 'ready':
        return orders.filter(o => o.status === 'READY');
      case 'completed':
        return orders.filter(o => o.status === 'COLLECTED' || o.status === 'REJECTED');
      default:
        return orders;
    }
  };
      ...
      <div className="container">
        ...
        <div style={styles.tabsContainer}>
          <button
            ...
            style={styles.tabBtn}
          >
            📋 Order Queue ({orders.filter(o => o.status !== 'COLLECTED' && o.status !== 'REJECTED').length})
          </button>
          <button
        ...
        {activeTab === 'queue' && (
          <div>
            ...
            <div style={styles.filterBar}>
              ...
              >
                In-Progress ({orders.filter(o => o.status === 'ACCEPTED' || o.status === 'PRINTING').length})
              </button>
              <button
                onClick={() => setQueueFilter('ready')}
                style={{
                  ...styles.filterBtn,
                  backgroundColor: queueFilter === 'ready' ? 'var(--neon-green)' : 'transparent',
                  fontWeight: queueFilter === 'ready' ? '700' : '500'
                }}
              >
                Ready for Collection ({orders.filter(o => o.status === 'READY').length})
              </button>
              <button
              ...
              >
                History ({orders.filter(o => o.status === 'COLLECTED' || o.status === 'REJECTED').length})
              </button>
            </div>
                  ...
                  <div key={order.id} className="card" style={styles.queueOrderCard}>
                    <div style={styles.orderHeader}>
                      <div>
                        ...
                        <h4 style={{fontSize: '1.25rem', marginTop: '0.15rem'}}>Student: {order.studentName}</h4>
                      </div>
                    ...
                    <div style={styles.orderBodyGrid}>
                      <div style={styles.fileDetailBlock}>
                        <strong>📄 File:</strong> {order.fileName} ({order.fileSize})
                      </div>
                      <div style={styles.specsBlock}>
                        <strong>Specs:</strong>{' '}
                        <span style={{textTransform: 'uppercase'}}>{order.printType}</span> |{' '}
                        {order.pageCount} pgs | {order.copies} copies |{' '}
                    ...
                    <div style={styles.orderFooter}>
                      <span style={styles.footerPrice}>
                        Total Calculated: <strong>₹{order.calculatedPrice.toFixed(2)}</strong>
                      </span>
                      ...
                      <div style={styles.actionsBlock}>
                        ...
                        {order.status === 'COLLECTED' && (
                          <span style={{color: '#008800', fontWeight: 'bold'}}>✓ Handed Over</span>
                        )}
                        {order.status === 'REJECTED' && (
